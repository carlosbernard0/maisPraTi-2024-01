...
import com.example.api_user.dto.LogiByIdDTO;
...
public class AuthController {
    ...
    public String login(@RequestBody LoginDTO loginDTO) {
        try {
            ...
            throw new RuntimeException("Invalid Credentials", error);
        }
    }

    @PostMapping("/loginId")
    public String loginById(@RequestBody LogiByIdDTO LogiByIdDTO) {
        try {
            // O AuthenticationManager realiza a autenticação baseada no nome de usuário e senha.
            // A autenticação é feita criando um UsernamePasswordAuthenticationToken com as credenciais fornecidas (username e password).
            Authentication authentication = authenticationManager.authenticate(
                    new UsernamePasswordAuthenticationToken(LogiByIdDTO.getId(), LogiByIdDTO.getPassword())
            );
